...
namespace EasyModbus
{
  public class ModbusClient
  {
    ...
    public void WriteMultipleCoils(int startingAddress, bool[] values)
    {
      ...
      this.startingAddress = BitConverter.GetBytes(startingAddress);
      //byte[] numArray1 = new byte[checked (16 + unchecked (values.Length % 8 != 0) ? unchecked (values.Length / 8) : unchecked (values.Length / 8) - 1)];
      byte[] numArray1 = new byte[checked (unchecked(values.Length % 8 != 0) ? unchecked(values.Length / 8) : unchecked(values.Length / 8) - 1)];
